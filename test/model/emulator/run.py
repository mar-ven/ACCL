...
def run_emulator(ranks: int, log_level: int, start_port: int, use_udp: bool, kernel_loopback: bool, hostfile: str):
    env = os.environ.copy()
    env['LOG_LEVEL'] = str(log_level)
    args = ['mpirun', '-np', str(ranks), '--tag-output', str(executable),
            'udp' if use_udp else 'tcp', str(start_port), "loopback" if kernel_loopback else "no_loopback"]
...
def main(ranks: int, log_level: int, start_port: int,
         use_udp: bool, kernel_loopback: bool, hostfile: str, build: bool):
    ...
    run_emulator(ranks, log_level, start_port, use_udp, kernel_loopback, hostfile)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hostfile', type=str, default=None,
                        help="File containing the IPs of emulator hosts")
    args = parser.parse_args()
    main(args.nranks, args.log_level, args.start_port, args.udp,
        not args.no_kernel_loopback, args.hostfile, not args.no_build)
